...
public interface OuvrierDesignationRepository extends JpaRepository<OuvrierDesignation, Integer> {
	...
	@Query("select ds.ouvrier.qualification  as qual , sum(ds.travail) as qt  from OuvrierDesignation ds WHERE"
			+ " ds.OuvrierFiche.projet.id=:projetID AND ds.travail IS NOT NULL AND ds.OuvrierFiche.date =:date group by ds.ouvrier.qualification ,ds.ouvrier.qualification.id ")
